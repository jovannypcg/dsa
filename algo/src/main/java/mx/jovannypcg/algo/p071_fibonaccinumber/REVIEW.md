...
**Code quality:** Clean split between the public API and the private recursive helper, matching the pattern used in [[p072_climbingstairs]]. Naming is clear aside from the leading underscore in `_fib` (the local variable), which is unconventional Java style but harmless.
